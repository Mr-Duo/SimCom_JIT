...
len_code = [len(code) for code in codes]
print(len(len_code))
result = Counter(len_code)
print(result)
n = {}
for k, v in result.items():
    v_ = f'{int(v / len(len_code) * 100)}%'
    n[k] = v_
...
print(len(len_tokens), len_tokens[:1000])
...
for k, g in groupby(sorted(len_tokens), key=lambda x: x//100):
    ...
    print(
        f'{k * 100}-{(k + 1) * 100 - 1}: {float(len(list(g)) / len(len_tokens))}'
    )
...
for k, g in groupby(sorted(msg_len), key=lambda x: x//50):
    print(f'{k * 50}-{(k + 1) * 50 - 1}: {float(len(list(g)) / len(msg_len))}')
# Dict Size of Msg
